...
from utils import progress, prompet, console
...
class Server:
    ...
    def refresh_cmd(self) -> None:
        self.cmdl.prompt = prompet(self.cmdl.client_path, self.active_client_data[-1], str(len(self.clients)))
    
    def listen(self) -> None:
        self.server.listen()
        thread: threading.Thread = threading.Thread(target=self._listen)
        thread.start()
        with progress(f"Listening on {ADDR} ..."):
    ...
    def send(self, msg: bytes) -> None:
        length: int = len(msg)
        self.active_client.send(f"{length}{' ' * (HEADER - length)}".encode(FORMAT))
